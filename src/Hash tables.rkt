...
(define productSum
  (lambda (values coefficients)
    (if ( = (length values) 1) (* (car values) (car coefficients))
        (+ (* (car values) (car coefficients)) (productSum (cdr values) (cdr coefficients))))))
(define hash
  (lambda (value coefficients size)
    (modulo (productSum (map char->integer (string->list value)) coefficients) size)))

(define (build-table coefficients)
  (let* ([freq-table (make-vector (* 2 (length zipcodes)) '())]
         [size 149])
    (begin
      (for ([r zipcodes])
        (let* ([ind (hash (second r) coefficients size)]
               [bucket (vector-ref freq-table ind)])
          (vector-set! freq-table ind (cons r bucket))
...
(define (reasonableZip? table state zip)
  (let ([index (hash state coefficients 149)])
    (zipInBucket state zip (vector-ref table index))))
...
(define (zipInBucket state zip bucket)
  (if (null? bucket)
      #f
      (let ([record (car bucket)])
        (if
         (and
          (not (null? record))
          (string=? state (cadr record))
          (>= zip (caddr record))
          (<= zip (cadddr record)))
         #t
         (zipInBucket state zip (cdr bucket))))))


(define coefficients '(17 32))
(define table (build-table coefficients))


(printf "Please enter a state in ALL caps and a zip code to see if it is reasonable.\n")
...
(let loop ()
  (begin
    ...
    (let ((result (reasonableZip? table a (string->number b))))
      (cond
        ((equal? result #t) (printf "The zip is reasonable\n"))
        (else
         (printf "The zip is NOT reasonable\n")))
      (loop)
      )))
